...
FROM
    llm_registry
WHERE
    lmarena_id IS NOT NULL
    AND lmarena_category IS NOT NULL ON CONFLICT (model_id, category) DO
UPDATE
SET
...
SELECT
    DISTINCT ON (lmarena_id) lmarena_id AS model_id,
    vendor,
    pricing_url,
    sync_id,
    last_updated
...
ALTER TABLE
    llm_registry_new RENAME TO llm_registry;

-- Step 6: add FK now that llm_registry has its final name
ALTER TABLE
    llm_ratings
ADD
    CONSTRAINT llm_ratings_model_id_fkey FOREIGN KEY (model_id) REFERENCES llm_registry (model_id);
