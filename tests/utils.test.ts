...
describe("logUserActivity", () => {
    it("inserts a user record and a stat entry", async () => {
        ...
        expect(stats[0]?.type).toBe("AI_CHAT");
    });
    ...
    it("assigns ADMIN role to the configured admin user", async () => {
        ...
        const [user] = await testDb.db
            .select()
            .from(users)
            .where(eq(users.userId, ADMIN_USER_ID));
    ...
    it("upserts the user on repeated calls, accumulating stat entries", async () => {
        ...
        expect(stats.every((s) => s.type === "AI_CHAT")).toBe(true);
    });
    ...
    it("silently skips messages with no 'from' field", async () => {
        const ctx = {
            from: undefined,
            message: { text: "ghost" },
        } as unknown as Parameters<typeof logUserActivity>[0];
        await logUserActivity(ctx);
...
describe("saveGeminiResponse", () => {
    it("updates the latest stat entry with the response text", async () => {
        await testDb.db
            .insert(users)
            .values({ userId: 77, username: "bob", role: "USER" });

        await testDb.db
            .insert(userStats)
            .values({ usersId: 77, input: "what is GPT-4?", type: "AI_CHAT" });

        await saveGeminiResponse(
            77,
            "GPT-4 is a large multimodal model by OpenAI.",
        );
        ...
        expect(stat?.response).toBe(
            "GPT-4 is a large multimodal model by OpenAI.",
        );
        expect(stat?.type).toBe("AI_CHAT");
    });
    ...
    it("does nothing when no stat record exists for the user", async () => {
        ...
        const stats = await testDb.db
            .select()
            .from(userStats)
            .where(eq(userStats.usersId, 9999));
...
describe("syncData", () => {
    ...
    it("updates tech registry scores from GitHub API", async () => {
        ...
        const techRows = await testDb.db.select().from(techRegistry);
